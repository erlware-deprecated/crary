...
handler(#aitch_req{uri = Uri, method = "GET"} = Req, BaseDir) ->
...
write_listing(Req, W, Dir, Names) ->
    ...
    aitch_body:write(W, [<<"  </tbody>
                             </table>
                            </div>
                            <div class=\"foot\">">>, aitch:ident(Req), <<"</div>
                           </body>
                          </html>">>]).
...
format_name(Req, Name, #file_info{type = Type}) ->
    ...
    [<<"<a href=\"">>,
     strip_slash(aitch:full_uri(Req)), $/, Name, TS, <<"\">">>,
     Name, TS, <<"</a>">>].
...
write_file(#aitch_req{opts = Opts} = Req, Path) ->
    case file:open(Path, [read, raw, binary]) of
	{ok, Fd} ->
	    BufSz = proplists:get_value(aitch_dir_listing_buffer_size,
					Opts, ?BUFSZ),
	    aitch:with_chunked_resp(Req, 200,
...
write_file(W, Fd, BufSz) ->
	...
	{ok, Data} ->
	    aitch_body:write(W, Data),
	    write_file(W, Fd, BufSz);
	eof ->
	    ok
    end.
    
has_index_file(#aitch_req{opts = Opts}, Path) ->
    has_index_file(proplists:get_value(aitch_dir_listing_index_file_names,
				       Opts, ?INDEX_NAMES),
		   Path);
