...
parse_header(RawHeader) ->
    case string:chr(RawHeader, $:) of
        0 -> % not fond
            throw({aitch_error, {bad_header, RawHeader}});
        Idx ->
...
get(Name, Headers) when is_list(Name) ->
    try gb_trees:get(Name, Headers)
            ...
            throw({aitch_headers_error, {field_name_not_found, Name}})
    end.
...
write(S, Headers) ->
    foreach(fun ({K, V}) -> aitch_sock:write(S, [K, $:, $\ , V, ?EOL]) end,
            Headers),
    % TODO: send timeouts (see manpage)
    aitch_sock:write(S, ?EOL).
